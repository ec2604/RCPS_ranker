...
RCPS_params = namedtuple('RCPS_parameters', ['delta', 'alpha', 'start_risk_bin'])
...
def run_experiment(model, params, data_type):
    model.train_model()
...
def plot_results(params, risk_list, interval_size_list, interval_size_capped_list, frac_abstained_list, result_dir,
    ...
    bins = np.linspace(params[data_type].start_risk_bin, params[data_type].alpha, 20).tolist() + [params[data_type].alpha + 0.01]
    ...
    ax[0].set_title(
        f'Risk at $ \\delta={params[data_type].delta}, \\alpha={params[data_type].alpha} $ Error={counts[-1] / counts_weighter:.2f}')
    ...
    ax[1].grid()
    plt.tight_layout()
    plt.savefig(result_dir + f'/{data_type}_risk_abstained.png')
    # plt.show()
    ...
    ax[1].grid()
    plt.tight_layout()
    plt.savefig(result_dir + f'/{data_type}_interval_sizes.png')


if __name__ == '__main__':
    params = {'university_rankings': RCPS_params(0.05, 0.01, 0.),
              'yearmsd': RCPS_params(0.05, 0.1, 0.05),
              'moviedata': RCPS_params(0.05, 0.1, 0.08)}
    for data_type in ['yearmsd', 'moviedata', 'university_rankings']:
        risk_list = []
        frac_abstained_list = []
        interval_size_list = []
        ...
        for i in tqdm(range(1000)):
            risk, frac_abstained, interval_size_capped, interval_size = run_experiment(model, params, data_type)
